...
char *read_line_stdin(void)
{
  ...
  // 2. Fetch an entire line from input stream stdin using getline() function. 
  ...
  if (line != NULL) {
    getline(&line,&buf_size,stdin); //https://c-for-dummies.com/blog/?p=1112
  }
...
char **tokenize_line_stdin(char *line)
{
  ...
  if (tokens != NULL){ //https://www.tutorialspoint.com/c_standard_library/c_function_strtok.htm
    token = strtok(line, SHELL_INPUT_DELIM); //whitespace delimiter
